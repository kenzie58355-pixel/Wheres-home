...
if (status === "attempted") return { border: "2px dotted #999", background: "transparent", color: "#777" };
...
if (status === "attempted") return { fontWeight: 500, opacity: 0.75, fontStyle: "italic" as const };
return { opacity: 0.55 };
};
...
attempted: "Attempted, but the plan didn't work out this time",
empty: "Not yet",
};
...
{c.status === "attempted" && <span style={{ position: "absolute", fontSize: "1rem", opacity: 0.6 }}>↩</span>}
</div>
...
<div style={{ width: "54px", height: "54px", borderRadius: "50%", border: selected.status === "attempted" ? "2px dotted #999" : "2px dashed var(--stamp-red)", color: selected.status === "attempted" ? "#777" : "var(--stamp-red)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Space Mono', monospace", fontSize: "0.7rem", marginBottom: "18px", transform: "rotate(-8deg)" }}>{selected.code}</div>
...
{selected.status === "attempted" ? "Read what happened →" : "Explore " + selected.name + " →"}
</Link>
) : (
